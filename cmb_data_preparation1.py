    ...
    for cz in range(num_patches_z):
        for cy in range(num_patches_y):
            for cx in range(num_patches_x):
                ...
                if frst is not None:
                    frst_patches[c, :patch.shape[0], :patch.shape[1], :patch.shape[2]] = frst[cmb_x_start:cmb_x_end, cmb_y_start:cmb_y_end, cmb_z_start:cmb_z_end]
                c += 1
...
def load_and_prepare_cmb_data_frst(filename, train='train', ps=32, priormap=None):
    ...
    for im in range(len(filename)):
        ...
        crop_org_data, coords = valdo_preprocessing.tight_crop_data(data)
                ...
                for i in range(0, 4):
                    ...
                    if chn2:
                        ...
                        augmented_img, augmented_frst, augmented_manseg = \
                            valdo_cmb_augmentations.augment2(image, frst, manmask)
                    else:
                        image = cmb_train[idx, :, :, :]
                        augmented_img, augmented_manseg = valdo_cmb_augmentations.augment(image, manmask)
...
def load_and_prepare_cmb_data(filename, train='train', ps=32, priormap=None):
    ...
    for im in range(len(filename)):
        ...
        crop_data = data1[coords[0]:coords[0] + coords[1], coords[2]:coords[2] + coords[3], coords[4]:coords[4] + coords[5]]
        labels = label[coords[0]:coords[0] + coords[1], coords[2]:coords[2] + coords[3], coords[4]:coords[4] + coords[5]]
        crop_brain = brain[coords[0]:coords[0] + coords[1], coords[2]:coords[2] + coords[3], coords[4]:coords[4] + coords[5]]
        crop_frst = frst[coords[0]:coords[0] + coords[1], coords[2]:coords[2] + coords[3],
                     coords[4]:coords[4] + coords[5]]
        crop_data[crop_org_data == 0] = 0.5
        ...
        if train == 'train':
            ...
            if priormap is not None:
                priormap = priormap[coords[0]:coords[0] + coords[1], coords[2]:coords[2] + coords[3], coords[4]:coords[4] + coords[5]]
...
def vessel_detection2d(inp_images2d):
    ...
    for slice_no in range(0, im1.shape[2]):
        ...
        frangi_output = frangi(imslice, scale_range=(0.5,1.2), scale_step=0.2, beta1=0.9, beta2=20, black_ridges=True) # (1, 3), 0.5, 100
...
def getting_cmb_data_disc_train(filename, patch_size=24):
    ...
    for im in range(len(filename)):
        ...
        all_cmb_patches = np.concatenate([all_cmb_patches, cmb_patches], axis=0) if all_cmb_patches.size else cmb_patches
        all_true_labels = np.concatenate([all_true_labels, true_labels], axis=0) if all_true_labels.size else true_labels
...
def getting_cmb_data_stonline_train(filename, patch_size=24):
    ...
    for im in range(len(filename)):
        ...
        cmb_patches = np.concatenate([detected_cmbpatches_img, detected_cmbpatches_frst], axis=-1)
        all_cmb_patches = np.concatenate([all_cmb_patches, cmb_patches], axis=0) if all_cmb_patches.size else cmb_patches
        all_gnd_patches = np.concatenate([all_gnd_patches, detected_cmbpatches_man], axis=0) if all_gnd_patches.size else detected_cmbpatches_man
        all_true_labels = np.concatenate([all_true_labels, true_labels], axis=0) if all_true_labels.size else true_labels
